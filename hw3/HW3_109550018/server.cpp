...
#define MAXLINE 1024
#define PORT 8888
#define PATH "/mnt/efs/fs1/status_record"
...
string check_status(string action) {
    string myText, ret = "", write = "";
    ...
    // action
    if (action == "check") {
        ret += "Server1: " + to_string(s1) + "\n";
        ret += "Server2: " + to_string(s2) + "\n";
        ret += "Server3: " + to_string(s3) + "\n";
            ...
            if (FD_ISSET(client_fds[f], &rset)) {
                ...
                if ((test = read(client_fds[f], buffer, sizeof(buffer))) == 0) {
                    ...
                    if (args[0] == "exit\n" || args[0] == "exit" || buffer[0] == '\0') {
                        ...
                        string temp = check_status("check");
                        char mr[MAXLINE]; strcpy(mr, temp.c_str());
                        write(client_fds[f], mr, strlen(mr));
                    }
                    // login
